...
let rec checkIfSumIsX inputList sum =
    let rec helper inputList sum acc = 
        match inputList with
        |x1::xs ->
            let firstRes = 
                xs 
                |> Seq.map (fun next ->
                    match x1 + next = sum with
                    |true -> Some(x1, next, x1 * next)
                    |_ -> None
                ) 
                |> Seq.choose id
                |> Seq.toList

            helper xs sum firstRes@acc

        |_ -> acc
    helper inputList sum []
...
let finalTest = checkIfSumIsX sample 2020
