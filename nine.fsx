...
you manage to connect it to your computer through the clever use 
of several paperclips. 
...
After that, each number you receive should be the sum of any two 
...
as it could be 1 plus 25 (or many other pairs, like 2 and 24).
49 would be a valid next number, as it is the sum of 24 and 25.
100 would not be valid; no two of the previous 25 numbers sum to 100.
50 would also not be valid; 
although 25 appears in the previous 25 numbers, 
...
let PreambleCuplesWhichSumToX preamble x : (int64*int64) seq =
    preamble 
    |> Seq.mapi (fun i p -> i,p) 
    |> Seq.collect (fun (i,current) ->
        let otherElements = 
            preamble 
            |> Set.ofSeq 
            |> Set.remove current 
...
let findFirstElementNotRespectingTheRule preambleWindowSize (input : int64 seq) =
    input
    |> Seq.skip preambleWindowSize
    |> Seq.mapi (fun i x -> i,x)
    |> Seq.tryFind (fun (i,x) -> 
         if input |> Seq.length < preambleWindowSize + i then
            false
         else
             let preamble = 
                input 
                |> Seq.skip i
                |> Seq.take preambleWindowSize
         
             PreambleCuplesWhichSumToX preamble x 
             |> Seq.isEmpty
    )
    |> Option.map (fun (i,x) -> x)

sampleInput
|> findFirstElementNotRespectingTheRule 5 //127

let solInput =
    System.IO.File.ReadLines(__SOURCE_DIRECTORY__ + "/nine.input.txt")
    |> Seq.toArray
    |> Seq.map (int64)

let sol1 =
    solInput
    |> findFirstElementNotRespectingTheRule 25

//part2
(*

The final step in breaking the XMAS encryption 
relies on the invalid number you just found: 
...
//awful imperative with mutation version (first version)
open System.Linq
let contiguousSetSumsToImperative (input : int64 seq) wrongNumber =
    let mutable contiguousList = new ResizeArray<int64>()
    let mutable skip = 0
    let mutable found = false //not operator doesnt work as expected in while loop
    let length = input |> Seq.length
    printfn $"length : {length}"
    while (skip < length && (found = false))  do
        try
            printfn $"skip:{skip}"
            let sequence = input |> Seq.skip skip 
            for el in sequence do
                contiguousList.Add(el)
                let sum = contiguousList.Sum()
                if sum > wrongNumber then
                    contiguousList.Clear()
                    failwith "break"
                else if sum = wrongNumber then
                    found <- true
                    failwith "break"
                else
                    ()
        with ex ->
            ...
            |_ -> 
                printfn $"{ex.Message}"
                found <- true //though list will be empty as this is a failure
        skip <- skip + 1
    contiguousList |> List.ofSeq        

//find a contiguous set of at least two numbers in your list 
//which sum to the invalid number from step 1.

let contiguousSetSumsTo (input : int64 seq) wrongNumber : int64 list =
    let results =
        input 
        |> Seq.mapi (fun i _ -> i) 
        |> Seq.map (fun i -> 
            ...
            |> Seq.fold (fun state el ->
                    let currentSum = state |> Seq.sum
                    if currentSum < wrongNumber then
                        el::state
                    else if (currentSum = wrongNumber) then
                        printfn $"{i},{state}"
                        state
                    else
                        []
...
contiguousSetSumsTo ([1;2;2;2;5;4;1;1;6;7] |> List.map (int64)) (6 |> int64)
        
        
let wrongNumberSample = 
    sampleInput
    |> List.map (int64)
    |> findFirstElementNotRespectingTheRule 5

let sol2Sample = 
    let contiguousSet = 
        contiguousSetSumsTo sampleInput (wrongNumberSample.Value)
    (contiguousSet |> Seq.min) + (contiguousSet |> Seq.max) //!62 
...
let sol2 = 
    let contiguousSet = 
        contiguousSetSumsTo solInput (wrongNumberSol.Value)
    if contiguousSet.Length >= 2 then
        Some((contiguousSet |> Seq.min) + (contiguousSet |> Seq.max))
    else
        None